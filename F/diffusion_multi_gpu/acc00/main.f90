...
program main
  ...
  do icnt = 0, nt-1
     ...
     call MPI_Send(f(1,1,nz)  , nx*ny, MPI_DOUBLE, rank_up  , tag, MPI_COMM_WORLD, ierr)
     call MPI_Recv(f(1,1,0)   , nx*ny, MPI_DOUBLE, rank_down, tag, MPI_COMM_WORLD, istat, ierr)

     call MPI_Send(f(1,1,1)   , nx*ny, MPI_DOUBLE, rank_down, tag, MPI_COMM_WORLD, ierr)
     call MPI_Recv(f(1,1,nz+1), nx*ny, MPI_DOUBLE, rank_up  , tag, MPI_COMM_WORLD, istat, ierr)
...
contains
  ...
  subroutine work_around(f,nx,ny,nz,rank_up,rank_down,tag,istat,ierr)
    ...
    !$acc host_data use_device(f)
    call MPI_Send(f(nx*ny*nz+1)    , nx*ny, MPI_DOUBLE, rank_up  , tag, MPI_COMM_WORLD, ierr)
    call MPI_Recv(f(1)             , nx*ny, MPI_DOUBLE, rank_down, tag, MPI_COMM_WORLD, istat, ierr)
    call MPI_Send(f(nx*ny+1)       , nx*ny, MPI_DOUBLE, rank_down, tag, MPI_COMM_WORLD, ierr)
    call MPI_Recv(f(nx*ny*(nz+1)+1), nx*ny, MPI_DOUBLE, rank_up  , tag, MPI_COMM_WORLD, istat, ierr)
    !$acc end host_data
