...
program main
  ...
  real(KIND=8),allocatable,dimension(:,:) :: sbufu,sbufd,rbufu,rbufd
  ...
  allocate(sbufu(nx,ny),sbufd(nx,ny),rbufu(nx,ny),rbufd(nx,ny))

  call init(nprocs, rank, nx, ny, nz, dx, dy, dz, f);

  call MPI_Barrier(MPI_COMM_WORLD, ierr)

  !$acc data copy(f,fn) create(sbufu,sbufd,rbufu,rbufd)
  tag = 0
#if MPI_TYPE==0
  ...
  do icnt = 0, nt-1
     ...
     flop = flop + diffusion3d_halo(nprocs, rank, nx, ny, nz, dx, dy, dz, dt, kappa, f, fn, sbufu, sbufd)
     !$acc wait(0)
     !$acc wait(1)
#if MPI_TYPE==0
     ...
     call work_around2(sbufu,sbufd,rbufu,rbufd,nx,ny,nz,rank_up,rank_down,tag,ireq,ierr)
#endif
     call MPI_Waitall(4,ireq,istat,ierr)
     !$acc wait(2)

     call halo_update(nx, ny, nz, fn, rbufu, rbufd)
  ...
  deallocate(f,fn)
  deallocate(sbufu,sbufd,rbufu,rbufd)
...
contains
  ...
  subroutine work_around2(sbufu,sbufd,rbufu,rbufd,nx,ny,nz,rank_up,rank_down,tag,ireq,ierr)
    implicit none
    real(kind=8), dimension(*) :: sbufu,sbufd,rbufu,rbufd
    integer :: nx,ny,nz,rank_up,rank_down,tag,ireq(:),ierr
    
    !$acc host_data use_device(sbufu,sbufd,rbufu,rbufd)
    call MPI_Irecv(rbufd(1), nx*ny, MPI_DOUBLE, rank_down, tag, MPI_COMM_WORLD, ireq(1), ierr)
    call MPI_Irecv(rbufu(1), nx*ny, MPI_DOUBLE, rank_up  , tag, MPI_COMM_WORLD, ireq(2), ierr)
    call MPI_Isend(sbufu(1), nx*ny, MPI_DOUBLE, rank_up  , tag, MPI_COMM_WORLD, ireq(3), ierr)
    call MPI_Isend(sbufd(1), nx*ny, MPI_DOUBLE, rank_down, tag, MPI_COMM_WORLD, ireq(4), ierr)
    !$acc end host_data
  end subroutine work_around2
