...
contains
  double precision function diffusion3d_halo(nprocs, rank, nx, ny, nz, dx, dy, dz, dt, kappa, f, fn, sbufu, sbufd)
    ...
    real(KIND=8),intent(out),dimension(1:nx,1:ny) :: sbufu,sbufd
    real(KIND=8) :: ce,cw,cn,cs,ct,cb,cc, tmp
    integer :: w,e,n,s,b,t
    integer :: i,j,k
    ...
    k = 1
    !$acc kernels async(0) present(f,fn,sbufd)
    !$acc loop collapse(2)
    do j = 1, ny
       do i = 1, nx
    ...
    k = nz
    !$acc kernels async(1) present(f,fn,sbufu)
    !$acc loop collapse(2)
    do j = 1, ny
       do i = 1, nx
          w = -1; e = 1; n = -1; s = 1; b = -1; t = 1;
  ...
  subroutine halo_update(nx, ny, nz, f, rbufu, rbufd)
    
    integer,intent(in) :: nx, ny, nz
    real(KIND=8),intent(in),dimension(1:nx,1:ny) :: rbufu,rbufd
    real(KIND=8),intent(out),dimension(1:nx,1:ny,0:nz+1) :: f
    integer :: i,j,k

    k = 0
    !$acc kernels async(2) present(f,rbufd)
    !$acc loop collapse(2)
    do j = 1, ny
       do i = 1, nx
          f(i,j,k) = rbufd(i,j)
    ...
    k = nz+1
    !$acc kernels async(2) present(f,rbufu)
    !$acc loop collapse(2)
    do j = 1, ny
       do i = 1, nx
          f(i,j,k) = rbufu(i,j)
  ...
  double precision function diffusion3d(nprocs, rank, nx, ny, nz, dx, dy, dz, dt, kappa, f, fn)
    ...
    !$acc kernels async(2) present(f, fn)
    !$acc loop collapse(3)
    do k = 2, nz-1
       do j = 1, ny
          do i = 1, nx
