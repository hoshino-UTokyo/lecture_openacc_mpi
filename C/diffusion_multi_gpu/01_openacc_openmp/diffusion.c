...
#pragma acc kernels present(fn,f) async(gpuid)
#pragma acc loop independent    
	for(int k = gpuid*nzgpu; k < (gpuid+1)*nzgpu; k++) {
#pragma acc loop independent        
	    for (int j = 0; j < ny; j++) {
#pragma acc loop independent
		for (int i = 0; i < nx; i++) {
	...
	int st = nx*ny*gpuid*nzgpu;
	int ln = nx*ny;
	fprintf(stdout,"gpuid = %d, st = %d, ln = %d, en = %d\n", gpuid, st, ln, st+ln-1);
	fflush(stdout);
	if(gpuid != 0) {
#pragma acc update host(fn[st:ln]) async(gpuid)
	}
	st = nx*ny*((gpuid+1)*nzgpu-1);
	ln = nx*ny;
	fprintf(stdout,"gpuid = %d, st = %d, ln = %d, en = %d\n", gpuid, st, ln, st+ln-1);
	fflush(stdout);
	if(gpuid != ngpus-1) {
...
#pragma omp parallel for
    for(int gpuid = 0; gpuid < ngpus; gpuid++) { 
        acc_set_device_num(gpuid, acc_device_nvidia);
	int st = nx*ny*(gpuid*nzgpu-1);
	int ln = nx*ny;
	fprintf(stdout,"gpuid = %d, st = %d, ln = %d, en = %d\n", gpuid, st, ln, st+ln-1);
	fflush(stdout);
	if(gpuid != 0) {
#pragma acc update device(fn[st:ln]) async(gpuid)
	}
	st = nx*ny*(gpuid+1)*nzgpu;
	ln = nx*ny;
	fprintf(stdout,"gpuid = %d, st = %d, ln = %d, en = %d\n", gpuid, st, ln, st+ln-1);
	fflush(stdout);
	if(gpuid != ngpus-1) {
